...
const _warnOnceCache = new Set();

export function warnOnce(message) {
  if (!_warnOnceCache.has(message)) {
    _warnOnceCache.add(message);
    warn(message);
  }
}

export function createElementNS(namespace, tagName, options = {}) {
  if (typeof document === 'undefined') {
    throw new Error('LXRN.coreUtils.createElementNS: document is not available');
  }
  if (typeof namespace !== 'string' || namespace.trim() === '') {
    throw new TypeError('LXRN.coreUtils.createElementNS: namespace must be a non-empty string');
  }
  if (typeof tagName !== 'string' || tagName.trim() === '') {
    throw new TypeError('LXRN.coreUtils.createElementNS: tagName must be a non-empty string');
  }
  return document.createElementNS(namespace, tagName, options);
}
