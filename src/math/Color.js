/**
 * @module Color
 * @description Color class using ColorSpace.
 * @author LXRN
 * @version 1.0.0
 */
...
export class Color {
    ...
    set colorSpace(value) { this._colorSpace = value; }
    ...
    setRGB(r, g, b, colorSpace = ColorSpace.SRGB) {
        this.r = clamp(r, 0, 1);
        this.g = clamp(g, 0, 1);
        this.b = clamp(b, 0, 1);
        this._colorSpace = colorSpace;
    ...
    setHSL(h, s, l, colorSpace = ColorSpace.SRGB) {
    ...
    copy(color) {
        ...
        this.g = color.g;
        this.b = color.b;
        this._colorSpace = color._colorSpace || ColorSpace.SRGB;
        return this;
    }
    ...
    getHex(colorSpace = ColorSpace.SRGB) {
        const temp = _color.copy(this);
        // Convert to target color space if needed
        if (this._colorSpace !== colorSpace) {
            // ColorSpace conversion would go here
        }
        return Math.round(clamp(temp.r * 255, 0, 255)) * 65536 +
               Math.round(clamp(temp.g * 255, 0, 255)) * 256 +
               Math.round(clamp(temp.b * 255, 0, 255));
    }
    ...
    getHSL(target, colorSpace = ColorSpace.SRGB) {
        const temp = _color.copy(this);
        // Convert to target color space if needed
        if (this._colorSpace !== colorSpace) {
            // ColorSpace conversion would go here
        }
        const r = temp.r, g = temp.g, b = temp.b;
    ...
    getRGB(target, colorSpace = ColorSpace.SRGB) {
        const temp = _color.copy(this);
        if (this._colorSpace !== colorSpace) {
            // ColorSpace conversion would go here
        }
